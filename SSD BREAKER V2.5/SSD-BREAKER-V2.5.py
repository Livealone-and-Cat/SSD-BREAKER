...
class SSD_BREAKER:
    ...
    def _cleanup_files(self, verbose: bool = True):
        ...
        # 清理当前文件和遗留文件
        for filepath in list(self.current_files) + [f for pattern in patterns for f in glob.glob(pattern)]:
            try:
    ...
    def _configure_operation(self) -> Optional[Tuple[float, Optional[float], Tuple[Optional[int], bool]]]:
        ...
        # 获取总大小
        while True:
            size_input = self._ssdbreaker_input("请输入要填充的总大小 (如 10G/500M/50%): ").strip()
            try:
                if '%' in size_input:
                    ...
                    print("必须包含单位！示例: 10G 或 50%")
            except (ValueError, IndexError):
                print("输入格式错误！示例: 10G 或 50%")
...
if __name__ == "__main__":
    ...
    try:
        import win32event
        mutex = win32event.CreateMutex(None, False, "SSD_BREAKER_Mutex")
